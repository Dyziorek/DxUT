...
//--------------------------------------------------------------------------------------
XMFLOAT3 CD3DArcBall::ScreenToVector( float fScreenPtX, float fScreenPtY )
{
    // Scale to screen
...
{
    XMVECTOR vPart;
	XMVECTOR fDot =  XMVector3Dot ( vFrom, vTo );
    vPart = XMVector3Cross( vFrom, vTo );

	vPart.m128_f32[3] = fDot.m128_f32[0];
...
void CD3DArcBall::OnMove( int nX, int nY )
{
    if( m_bDrag )
    {
		...
		XMVECTOR vecTmp = XMLoadFloat4(&m_qDown);
		vecTmp = XMQuaternionMultiply(vecTmp, QuatFromBallPoints( XMLoadFloat3(&m_vDownPt), XMLoadFloat3(&m_vCurrentPt) ));
		XMStoreFloat4(&m_qNow, vecTmp);
    }
}
...
CBaseCamera::CBaseCamera()
{
    ...
    XMFLOAT3 vEyePt = XMFLOAT3( 0.0f, 0.0f, 0.0f );
    XMFLOAT3 vLookatPt = XMFLOAT3( 0.0f, 0.0f, 1.0f );
...
VOID CBaseCamera::SetViewParams( XMFLOAT3* pvEyePt, XMFLOAT3* pvLookatPt )
{
    ...
    XMFLOAT3 vUp( 0,1,0 );
	m_mView = XMMatrixLookAtLH(  XMLoadFloat3(pvEyePt), XMLoadFloat3(pvLookatPt), XMLoadFloat3(&vUp) );
...
void CBaseCamera::UpdateMouseDelta()
{
    ...
    if( m_bResetCursorAfterMove )
    {
		...
		GetMonitorInfo(MonitorFromWindow(m_hCameraWnd,MONITOR_DEFAULTTONEAREST),&mi);
	...
	XMStoreFloat2(&m_vRotVelocity, XMLoadFloat2(&m_vMouseDelta) * m_fRotationScaler);
}
...
{
    XMMATRIX mRotDelta;
	XMStoreFloat2(&m_vRotVelocity, DirectX::XMLoadFloat2(&m_vMouseDelta) * m_fRotationScaler * 0.02f);

    XMVECTOR vAccel = XMLoadFloat3(&m_vKeyboardDirection);
    ...
    if( m_bMovementDrag )
    {
		...
		if( XMVector3Length(vAccel = XMVector3LengthSq( vAccel )).m128_f32[0] > 0 )
        {
        ...
        else
        {
            ...
            if( m_fDragTimer > 0 )
            {
                // Drag until timer is <= 0
				 XMLoadFloat3(&m_vVelocity) -= XMLoadFloat3( &m_vVelocityDrag) * fElapsedTime;
...
//--------------------------------------------------------------------------------------
void CBaseCamera::ConstrainToBoundary( XMFLOAT3* pV )
{
...
VOID CFirstPersonCamera::FrameMove( FLOAT fElapsedTime )
{
	...
	XMVECTOR veloc = XMLoadFloat3(&m_vVelocity);
	veloc = veloc * fElapsedTime;

    // Simple euler method to calculate position delta
	XMFLOAT3 vPosDelta;
	XMStoreFloat3(&vPosDelta, veloc);
    ...
    XMFLOAT3 vWorldUp, vWorldAhead;
    XMFLOAT3 vLocalUp = XMFLOAT3( 0, 1, 0 );
    XMFLOAT3 vLocalAhead = XMFLOAT3( 0, 0, 1 );
    XMStoreFloat3(&vWorldUp, XMVector3TransformCoord( XMLoadFloat3(&vLocalUp), mCameraRot ));
    XMStoreFloat3(&vWorldAhead, XMVector3TransformCoord( XMLoadFloat3(&vLocalAhead), mCameraRot ));

    // Transform the position delta by the camera's rotation 
    XMFLOAT3 vPosDeltaWorld;
    if( !m_bEnableYAxisMovement )
    {
    ...
    XMStoreFloat3(&vPosDeltaWorld , XMVector3TransformCoord( XMLoadFloat3(&vPosDelta), mCameraRot ));

    // Move the eye position 
    XMStoreFloat3(&m_vEye,  (XMLoadFloat3(&m_vEye) += XMLoadFloat3(&vPosDeltaWorld)));
    if( m_bClipToBoundary )
        ConstrainToBoundary( &m_vEye );

    // Update the lookAt position based on the eye position 
    XMStoreFloat3(&m_vLookAt ,  XMLoadFloat3(&m_vEye) + XMLoadFloat3(&vWorldAhead));

    // Update the view matrix
    m_mView = XMMatrixLookAtLH( XMLoadFloat3(&m_vEye), XMLoadFloat3(&m_vLookAt), XMLoadFloat3(&vWorldUp) );

    m_mCameraWorld = XMMatrixInverse( NULL, m_mView );
}
...
VOID CModelViewerCamera::FrameMove( FLOAT fElapsedTime )
{
	...
	XMVECTOR locLookAt = XMLoadFloat3(&m_vLookAt);
	locLookAt += vPosDeltaWorld;
    XMStoreFloat3(&m_vLookAt, locLookAt);
    if( m_bClipToBoundary )
        ConstrainToBoundary( &m_vLookAt );

    // Update the eye point based on a radius away from the lookAt position
    XMStoreFloat3(&m_vEye, XMLoadFloat3(&m_vLookAt) - vWorldAhead * m_fRadius);

    // Update the view matrix
    m_mView = XMMatrixLookAtLH( XMLoadFloat3(&m_vEye), XMLoadFloat3(&m_vLookAt), vWorldUp );
    ...
    XMVECTOR* pXBasis = &m_mModelRot.r[0];
    XMVECTOR* pYBasis = &m_mModelRot.r[1];
    XMVECTOR* pZBasis = &m_mModelRot.r[2];
    ...
    *pZBasis = XMVector3Cross( *pXBasis, *pYBasis );
	...
	m_mModelRot.r[3].m128_f32[0] = m_vLookAt.x;
    m_mModelRot.r[3].m128_f32[1] = m_vLookAt.y;
    m_mModelRot.r[3].m128_f32[2] = m_vLookAt.z;

    // Translate world matrix so its at the center of the model
    XMMATRIX mTrans;
    mTrans = XMMatrixTranslation( -m_vModelCenter.x, -m_vModelCenter.y, -m_vModelCenter.z );
...
void CModelViewerCamera::SetViewParams( XMFLOAT3* pvEyePt, XMFLOAT3* pvLookatPt )
{
    ...
    XMMATRIX mRotation;
    XMFLOAT3 vUp( 0,1,0 );
    mRotation = XMMatrixLookAtLH(  XMLoadFloat3(pvEyePt), XMLoadFloat3(pvLookatPt), XMLoadFloat3(&vUp) );
	quat = XMQuaternionRotationMatrix( mRotation );
	XMFLOAT4 quatResult;
	XMStoreFloat4(&quatResult, quat);
	m_ViewArcBall.SetQuatNow( quatResult );

    // Set the radius according to the distance
    XMVECTOR vEyeToPoint;
	vEyeToPoint = XMVectorSubtract( XMLoadFloat3(pvLookatPt), XMLoadFloat3(pvEyePt) );
	SetRadius(   XMVector3Length( vEyeToPoint ).m128_f32[0] );
